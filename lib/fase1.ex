defmodule Fase1 do
  ...
  def container_info(id) do
    case container_exists(id) do
      ...
      %{id: id, imagen: imagen, name: name} ->
        IO.puts("Contenedor #{id}")
        IO.puts("----------------------------")
        IO.puts("#{String.pad_trailing("Id", 15)}: #{id}")
        IO.puts("#{String.pad_trailing("Imagen", 15)}: #{imagen}")
        IO.puts("#{String.pad_trailing("Nombre", 15)}: #{name}")
    end
  end
  ...
  def detener_eliminar_contenedor(id) do
    case System.cmd("docker", ["stop", id]) do
      {_, 0} ->
        case System.cmd("docker", ["rm", id]) do
          {_, 0} -> IO.puts("El contenedor fue detenido y eliminado")
          {error, _} -> IO.puts("Hubo un error al eliminar el contenedor: #{error}")
        end

      {_error, _} ->
        case System.cmd("docker", ["rm", id]) do
          {_, 0} -> IO.puts("El contenedor fue detenido ")
          {error, _} -> IO.puts("Hubo un error al eliminar el contenedor: #{error}")
        end
    end
  end
